...
int find_id( Graph graph, char *name){
    int i = INFINITIVE_VALUE;
    int id;
    JRB ds = graph->vertices;
    JRB lst;
    jrb_traverse( lst, ds){
        id = jval_i( lst->key);
        if( strcmp( name, getVertex( graph, id)) == 0){
            i = id;
            break;
        }
    }
...
Graph copyGraph( Graph g, int size){
    ...
    jrb_traverse( lst, root){
        ...
        num = outdegree( g, id, out);
        for( i = 0; i < num; i++){
            addEdge( h, id, out[i], getEdgeValue( g, id, out[i]));
        }
    }
...
void dropGraph(Graph graph)
{
	...
	jrb_traverse(node,graph->vertices){
		...
		jrb_free_tree((JRB)detailnode->detail);
		jrb_free_tree((JRB)detailnode->linethong);
	}
...
void addDetail( Graph g, int id, void *detail){
    JRB vnode = jrb_find_int(g->vertices,id);
	JRB deNode;
	if(vnode==NULL) {
		return;
    }
	detailVertex iver = (detailVertex) vnode->val.v;
	deNode = iver->detail;
    jrb_insert_int( deNode, iver->stt, new_jval_v( detail));
    iver->stt = iver->stt + 1;
}
